...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
    body { display:flex; background:#f5f8fa; height:100vh; }

    /* Sidebar */
    .menu { flex:1; padding:10px 0; }
    .menu a { display:flex; align-items:center; padding:12px 20px; color:#333; text-decoration:none; font-size:15px; transition:background 0.2s; }
    .menu a i { margin-right:10px; color:#007B8F; }
    .menu a:hover, .menu a.active { background:#e6f7f9; color:#007B8F; border-left:4px solid #007B8F; }

    /* Main content */
    .main { flex:1; display:flex; flex-direction:column; }
    .topbar { display:flex; justify-content:flex-end; align-items:center; background:#fff; padding:10px 20px; border-bottom:1px solid #ddd; }
    .topbar i { margin-left:20px; font-size:18px; cursor:pointer; }

    /* Profile header */
    .profile-header { background:#007B8F; color:#fff; padding:20px; border-radius:8px; margin:20px; display:flex; justify-content:space-between; align-items:center; }
    .profile-header .info { display:flex; align-items:center; }
    .profile-header .info i { font-size:50px; margin-right:15px; }
    .profile-header button { background:#0ca4b9; border:none; padding:10px 20px; border-radius:5px; color:#fff; cursor:pointer; }

    /* Tabs */
    .tabs { margin:0 20px; background:#fff; border-radius:8px; padding:15px; }
    .tab-buttons { display:flex; border-bottom:1px solid #ddd; margin-bottom:15px; }
    .tab-buttons button { background:none; border:none; padding:10px 15px; cursor:pointer; font-weight:500; color:#555; }
    .tab-buttons button.active { border-bottom:3px solid #007B8F; color:#007B8F; }
    .tab-content { display:none; }
    .tab-content.active { display:block; }

    /* Form sections */
    .section { margin-bottom:20px; }
    .section h4 { color:#007B8F; margin-bottom:10px; }
    .info-grid { display:grid; grid-template-columns:repeat(2, 1fr); gap:15px; }
    .info-grid div { display:flex; flex-direction:column; }
    .info-grid label { font-size:13px; color:#666; margin-bottom:4px; }
    .info-grid input, .info-grid select, .info-grid textarea { padding:8px; border:1px solid #ddd; border-radius:5px; background:#f9f9f9; }
