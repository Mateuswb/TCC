...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .save-btn{background: linear-gradient(180deg,#4f46e5,#3b82f6);color:#fff;padding:10px 16px;border-radius:10px;border:0;
    box-shadow:0 8px 18px rgba(59,130,246,0.18);cursor:pointer;font-weight:600;display:inline-flex;gap:10px;align-items:center;}
...
<body>
  <div class="page">
    ...
    <div class="sheet">
      <div class="sheet-header">
        <div></div>
        <div>
          <button class="save-btn" id="btnSave">Salvar Planilha</button>
        </div>
      </div>
  ...
  <!-- small help card --> <div class="sheet" style="padding:16px; width: 70%; margin-left: 250px;"> <strong>Como usar a planilha:</strong> <p class="small-muted" style="margin:6px 0 0">Preencha os campos <em>Início</em> e <em>Fim</em> dos dias desejados. Use <strong>+</strong> para adicionar um período extra no final da planilha, ou o ícone de <strong>clonagem</strong> para replicar os valores da linha selecionada para todos os dias. Clique <strong>Salvar Planilha</strong> para continuar.</p> </div> </div>

<script>
(function(){
  const weekdays = [
    { short:'SEG', full:'Segunda-feira', color:'#ef4444' },
    { short:'TER', full:'Terça-feira', color:'#f97316' },
    { short:'QUA', full:'Quarta-feira', color:'#f59e0b' },
    { short:'QUI', full:'Quinta-feira', color:'#10b981' },
    { short:'SEX', full:'Sexta-feira', color:'#06b6d4' },
  ...
  function render(){
    ...
    rows.forEach(r=>{
      ...
      dayCell.innerHTML = `<div class="day-cell">
        <span class="day-dot" style="background:${day.color}"></span>
        <div><div class="day-title">${day.short}</div><div class="day-sub">${day.full}</div></div>
      </div>`;

      function inputCell(value, name){
        const td = document.createElement('td');
        td.className = 'col-time';
        td.innerHTML = `<div class="time-card"><div class="time-input"><input type="time" value="${value}" data-field="${name}"/></div></div>`;
        td.querySelector('input').addEventListener('input', e => r[name] = e.target.value);
        return td;
      }

      const tdStart = inputCell(r.start,'start');
      const tdEnd   = inputCell(r.end,'end');
      const tdIS    = inputCell(r.iStart,'iStart');
      const tdIE    = inputCell(r.iEnd,'iEnd');

      const tdActions = document.createElement('td');
      tdActions.innerHTML = `<button class="btn-small btn-clone" data-action="clone">⎘</button>`;
